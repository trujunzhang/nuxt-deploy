...
interface IF8PhotosSingleTopProps {
  objectSchemaName: string
  photosListTask: IListWithPhotosDictTask
  forObject: any
}

interface IF8PhotosSingleTopState {
  modelType: string
}

export class F8PhotosSingleTop extends React.Component<
  IF8PhotosSingleTopProps,
  IF8PhotosSingleTopState
> {
  constructor(props: IF8PhotosSingleTopProps, context) {
    super(props)
  ...
  render() {
    const { objectSchemaName, photosListTask, forObject } = this.props
        ...
        <div className="section-header media-header">
          <div className="arrange arrange--12 arrange--bottom">
            <div className="arrange_unit arrange_unit--fill">
              ...
              <h1 className="media-header_title h2">{`${photos.length} photos for ${
                forObject.displayName
              }`}</h1>
            </div>

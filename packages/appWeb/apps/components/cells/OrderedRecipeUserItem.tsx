...
interface IOrderedRecipeUserItemProps {
  listTask: IListWithPhotosDictTask
  user: IParseModelUsers
}

@(withRouter as any)
export class OrderedRecipeUserItem extends React.Component<IOrderedRecipeUserItemProps, {}> {
  renderLeft() {
    const { listTask, user } = this.props
    return (
    ...
    return (
      <div className="media-story">
        ...
        <div className="tag-18x18_flame-dd5114">
          <small>
            <Telescope.FormattedRelative value={user.updatedAt} />
          </small>
        </div>
      </div>
    )

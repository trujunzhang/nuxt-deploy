...
class ParamsHelper {
  ...
  static String getOnlinePageViewPath(int selectedIndex,
    ...
    return '${Routes.ONLINE_PHOTO_PAGE}?${ParamsHelper.PHOTO_TYPE}=${photoType.vnText}&${ParamsHelper.RELATED_ID}=$relatedId&${ParamsHelper.SELECTED_INDEX}=$selectedIndex';
  }

  // Reviews
  static String getNewReviewPath(
      {required ReviewType reviewType, required String relatedId}) {
    return '${Routes.EDIT_REVIEW}?${ParamsHelper.REVIEW_TYPE}=${reviewType.vnText}&${ParamsHelper.RELATED_ID}=$relatedId';
  }

  static String getReviewListPath(
      {required ReviewType reviewType, required String relatedId}) {
    return '${Routes.REVIEWS_LIST}?${ParamsHelper.REVIEW_TYPE}=${reviewType.vnText}&${ParamsHelper.RELATED_ID}=$relatedId';
  }

  // Photos
  static String getTakeCameraPath(
      {required PhotoType photoType, required String relatedId}) {
    return '${Routes.TAKE_CAMERA}?${ParamsHelper.PHOTO_TYPE}=${photoType.vnText}&${ParamsHelper.RELATED_ID}=$relatedId';
  }
  ...
  static String getNewPhotoPath(
    ...
    return '${Routes.NEW_PHOTO}?${ParamsHelper.PHOTO_TYPE}=${photoType.vnText}&${ParamsHelper.RELATED_ID}=$relatedId&${ParamsHelper.IMG_PATH}=$imgPath';
  }
  ...
  static String getOnlinePhotoGridViewPath({
    ...
    return '${Routes.ONLINE_PHOTO_GRID}?${ParamsHelper.PHOTO_TYPE}=${photoType.vnText}&${ParamsHelper.RELATED_ID}=$relatedId';
  }

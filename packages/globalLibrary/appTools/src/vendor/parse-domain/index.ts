...
const knownTlds = require('./tld')

const urlParts = /^(https?:\/\/)?([^/]*@)?(.+?)(:\d{2,5})?([/?].*)?$/ // 1 = protocol, 2 = auth, 3 = domain, 4 = port, 5 = path
const dot = /\./g

function matchTld(domain: any, options: any) {
  let tld = null
 ...
 * @returns {Object|null}
 */
export function parseDomain(url: string = '', options: any = {}): IParseDomainWithNull {
  const normalizedUrl: any = Normalize.url(url)
  let tld: any = null
  let urlSplit: any
  let domain: any
